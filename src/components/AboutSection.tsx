...
export default function AboutSection() {
  ...
  return (
    <section ref={sectionRef} id="about" className="relative py-28 bg-[#FDFBF7] bg-pattern-dots overflow-hidden">
      {/* Background glow */}
      <div className="absolute top-1/2 left-1/4 w-[600px] h-[600px] -translate-y-1/2 rounded-full opacity-[0.03]"
        style={{ background: "radial-gradient(circle, var(--gold), transparent 70%)", filter: "blur(60px)" }} />

      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
        {/* Left Content */}
        <div>
          <div className="about-badge inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full border border-gray-200 bg-gray-50">
            <span className="text-[var(--gold)] text-xs">✦</span>
            <span className="text-[var(--gold)] text-xs tracking-[0.3em] uppercase font-medium">Our Story</span>
            <span className="text-[var(--gold)] text-xs">✦</span>
          </div>

          <h2 className="about-heading text-5xl md:text-6xl font-bold mb-6 leading-tight" style={{ fontFamily: "'Playfair Display', serif", color: "var(--gold)" }}>
            A Legacy of<br />
            <span style={{ color: "var(--foreground)" }}>Flavours &</span><br />
            Tradition
          </h2>
          ...
          <div className="about-text flex gap-10 mt-10">
            <div>
              ...
              <p className="text-gray-500 text-sm tracking-wider uppercase mt-1">Happy Reviews</p>
            </div>
            <div className="w-px bg-gray-200" />
            <div>
              <p className="text-4xl font-bold gold-text" style={{ fontFamily: "'Playfair Display', serif" }}>₹20</p>
              <p className="text-gray-500 text-sm tracking-wider uppercase mt-1">Starting Price</p>
            </div>
            <div className="w-px bg-gray-200" />
            <div>
              <p className="text-4xl font-bold gold-text" style={{ fontFamily: "'Playfair Display', serif" }}>3.9★</p>
              <p className="text-gray-500 text-sm tracking-wider uppercase mt-1">Google Rating</p>
            </div>
          </div>
        </div>
        ...
        <div className="about-image-wrap">
          <div className="about-values-grid grid grid-cols-2 gap-4">
            {values.map((v, i) => (
              <div key={i} className="about-value-card menu-card p-6 rounded-2xl border border-gray-200 bg-card">
                <div className="text-3xl mb-3">{v.icon}</div>
                <h4 className="font-bold text-gray-900 mb-2 text-sm tracking-wide uppercase" style={{ fontFamily: "'Playfair Display', serif" }}>
                  {v.title}
                </h4>
          ...
          {/* Float Badge */}
          <div className="float-badge mt-6 p-4 rounded-2xl border border-gray-200 bg-gray-50 flex items-center gap-4">
            <div className="text-3xl">🏆</div>
            <div>
              <p className=" text-gray-900 font-semibold text-sm">Justdial Verified</p>
              <p className="text-gray-500 text-xs">Rated 3.8 · 1139+ reviews on Justdial</p>
            </div>
